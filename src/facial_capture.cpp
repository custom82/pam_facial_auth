...
int main(int argc, char** argv) {
    ...
    if (clean_only || force_restart) {
        std::cout << "[*] Pulizia dati utente: " << user << std::endl;
        fa_clean_captures(user, cfg, log);
        if (clean_only) return 0;
    }

    std::cout << "[*] Avvio acquisizione per: " << user << " (Detector: " << cfg.detector << ")" << std::endl;
    ...
    std::cout << "[SUCCESS] Operazione completata." << std::endl;
    return 0;
}
