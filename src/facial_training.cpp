...
static void print_help()
{
    ...
    "  -n, --frames N         Override number of frames (used by capture phase)\n"
    "  -s, --sleep MS         Delay between frames (used by capture phase)\n"
...
int facial_training_cli_main(int argc, char *argv[])
{
    ...
    // ------------------------------------------------------------
    // Parse args (SOLO store, niente logica)
    // ------------------------------------------------------------
    ...
    if (opt_reset) {
        ...
        if (std::filesystem::exists(mdl_path))
            std::filesystem::remove(mdl_path);
        return 0;
    }
    ...
    // ------------------------------------------------------------
    // Train (delegato completamente alla libreria)
    // ------------------------------------------------------------
