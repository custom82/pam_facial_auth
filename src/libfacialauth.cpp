...
extern "C" {
    ...
    FA_EXPORT bool fa_load_config(FacialAuthConfig& cfg, std::string& log, const std::string& path) {
        ...
        while (std::getline(file, line)) {
            ...
            if (std::getline(is_line, key, '=') && std::getline(is_line, value)) {
                ...
                else if (key == "detect_yunet") { cfg.detect_yunet = value; cfg.cascade_path = value; } // Allineamento plugin_sface
    ...
    FA_EXPORT bool fa_capture_user(const std::string& user, const FacialAuthConfig& cfg, const std::string& device_path, std::string& log) {
        cv::VideoCapture cap;
        if (device_path.find_first_not_of("0123456789") == std::string::npos) cap.open(std::stoi(device_path));
        else cap.open(device_path);
